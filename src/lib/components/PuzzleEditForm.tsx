...
async function send(
    ...
    // ヒントを更新
    const hintHtmls = hintQuills.map(
        (hintQuill) => hintQuill.current?.root.innerHTML ?? ""
    );
...
export default function PuzzleEditForm({ id }: { id: string }) {
    ...
    useEffect(() => {
        async function loadQuill() {
            ...
            const descriptionDelta = quill.clipboard.convert({
                html: puzzle.description,
            });
            const solutionDelta = quill.clipboard.convert({
                html: puzzle.solution,
            });
    ...
    useEffect(() => {
        import("quill").then((Quill) => {
            ...
            hints.forEach((hint, index) => {
                const hintsDelta = quill.clipboard.convert({
                    html: hint.content,
                });
